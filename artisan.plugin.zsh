...
function artisan() {
    ...
    if [ "$docker_compose_config_path" = '' ]; then
        ...
        if [ "`grep "laravel/sail" $docker_compose_config_path | head -n1`" != '' ]; then
            ...
            local docker_compose_service_name=${ARTISAN_AUT_CMPL_DOCKER_SERVICE:-$($docker_compose_cmd ps --services 2>/dev/null | grep 'app\|php\|api\|workspace\|laravel\.test\|webhost' | head -n1)}
